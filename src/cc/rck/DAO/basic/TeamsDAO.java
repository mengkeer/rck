...
/**
 * A data access object (DAO) providing persistence and search support for Teams
...
public class TeamsDAO extends BaseHibernateDAO {
	...
	public Teams findById(java.lang.Integer id) {
		log.debug("getting Teams instance with id: " + id);
		try {
			Teams instance = (Teams) getSession().get("cc.rck.model.Teams", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Teams instance) {
		...
		try {
			List results = getSession().createCriteria("cc.rck.model.Teams")
